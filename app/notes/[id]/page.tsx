...
function NoteDetails({ params }: any) {
  ...
  const getComments = async () => {
    const { data } = await supabase
      .from("comments")
      .select(`*, note_users(email, username)`)
      .eq("deleted", false)
      .eq("note_id", id);
    setComments(data);
    setLoading(false);
  ...
  return (
    <div>
      {note && user ? (
              ...
              {comments && comments.length > 0 ? (
                comments.map((comment) => (
                  ...
                  >
                    ...
                    <div>
                      <p className="text-xs">
                        {comment.note_users && comment.note_users.username
                          ? comment.note_users.username
                          : comment.note_users.email}
                      </p>
                      <p className=" text-xs">
                        {new Intl.DateTimeFormat("en-US", {
                          dateStyle: "short",
        ...
        <div className="flex flex-col gap-5 w-full h-[70vh] justify-center items-center">
          <p>Note not found</p>
